...
public class TerminalCommands
{
    ...
    {
        Plugin.Logger.LogInfo("Setting up tracker command");
        // Tracker command
        TerminalCommandBasicInformation trackerCommandInfo = new TerminalCommandBasicInformation("Tracker", "Archipelago", "Shows all logically reachable checks", ClearText.Result | ClearText.Query);
        ...
        Plugin.Logger.LogInfo("Setting up progress command");
        // Progress command
        TerminalCommandBasicInformation progressCommandInfo = new TerminalCommandBasicInformation("Progress", "Archipelago", "Shows current progress towards the chosen goal", ClearText.Result | ClearText.Query);
...
Quota: {((Quota)MwState.Instance.GetLocationMap("Quota")).GetTrackerText()}, {totalQuota % moneyPerQuota}/{moneyPerQuota}";
        ...
        Plugin.Logger.LogInfo("Setting up scrap command");
        // Scrap command
        TerminalCommandBasicInformation scrapCommandInfo = new TerminalCommandBasicInformation("Scrap", "Archipelago", 
        ...
        DawnLib.DefineTerminalCommand(NamespacedKey<DawnTerminalCommandInfo>.From("aplc", "scrap_command"), scrapCommandInfo, builder =>
        {
            ...
            {
                inputBuilder.SetResultDisplayText(GetScrapLogic);
            });
        });

        Plugin.Logger.LogInfo("Setting up hint command");
        // Hint command
        TerminalCommandBasicInformation hintCommandInfo = new TerminalCommandBasicInformation("Hints", "Archipelago", "Shows all received hints that haven't yet been completed.", ClearText.Result | ClearText.Query);
        ...
        Plugin.Logger.LogInfo("Setting up config command");
        // Config command
        TerminalCommandBasicInformation configCommandInfo = new TerminalCommandBasicInformation("Config", "Archipelago", "Shows and sets the value of config settings for APLC.", ClearText.Result | ClearText.Query);    // not working
        DawnLib.DefineTerminalCommand(NamespacedKey<DawnTerminalCommandInfo>.From("aplc", "config_command"), configCommandInfo, builder =>
        {
            ...
            builder.DefineInputCommand(inputBuilder =>
            {
                inputBuilder.SetResultDisplayText((text) =>
                {
                    if (text == "")
                    {
...
";
        ...
        Plugin.Logger.LogInfo("Setting up filler command");
        // Filler command
        TerminalCommandBasicInformation fillerCommandInfo = new TerminalCommandBasicInformation("Filler", "Archipelago", "See and use available filler items.", ClearText.Result | ClearText.Query);    // not working
        DawnLib.DefineTerminalCommand(NamespacedKey<DawnTerminalCommandInfo>.From("aplc", "filler_command"), fillerCommandInfo, builder =>
        {
            ...
            {
                complexQueryBuilder.SetContinueOrCancelDisplayTexts(() =>
                {
...
To use a filler item, enter the item's name";
                });
                complexQueryBuilder.SetContinueKeywords(["More Time", "Clone Scrap", "Birthday Gift", "Money"]);
                complexQueryBuilder.SetResultDisplayTexts([() => TryUseFiller("More Time"), () => TryUseFiller("Clone Scrap"), () => TryUseFiller("Birthday Gift"), () => TryUseFiller("Money")]);
                complexQueryBuilder.SetCancelDisplayText(() => "bruh");
                complexQueryBuilder.SetCancelKeyword("cancel");
            });*/
            builder.DefineInputCommand(inputBuilder =>
            {
                inputBuilder.SetResultDisplayText((text) =>
...
To use a filler item, re-enter this command followed by the item's name.";
                    ...
                    foreach (var fillerName in fillerNames)
                    {
                        ...
                        {
                            return fillerMap.Use() ? $"Used {fillerName} successfully" : $"Failed to use {fillerName}. Make sure you are in an area where the filler makes sense to use.";
                        }
        ...
        Plugin.Logger.LogInfo("Setting up world command");
        // Filler command
        TerminalCommandBasicInformation worldCommandInfo = new TerminalCommandBasicInformation("World", "Archipelago", "USE ONLY FOR ADVANCED CUSTOM CONTENT - CAN BREAK SAVES ---- Use to set the custom apworld's name to connect to custom apworlds\nUseage: 'world [world name]'", ClearText.Result | ClearText.Query);    // not working
    ...
    internal static string GetScrapLogic(string text)
    {
        if (text == "")
        {
    ...
    internal static string TryUseFiller(string fillerName)
    {
        if (MultiworldHandler.Instance == null)
        {
            return "Not connected to the multiworld.";
        ...
        if (!MwState.Instance.TryGetItemMap<FillerItems>(fillerName, out FillerItems fillerMap)) return $"{fillerName} is not a valid filler item";
        if (fillerMap.GetReceived() > fillerMap.GetUsed())
        {
            return fillerMap.Use() ? $"Used {fillerName} successfully" : $"Failed to use {fillerName}. Make sure you are in an area where the filler makes sense to use.";
        }
